...
class Reportico extends ReporticoObject
{
    ...
    public function &createGraph()
    {
        ...
        if (!class_exists("reportico_graph", false)) {
            switch ($engine) {
                ...
                case "JPGRAPH":
                    $graph = new ChartJpgraph($this, "internal");
                    break;
    ...
    public function getExecuteMode()
    {
        ...
        if (array_key_exists('execute_mode', $_REQUEST)) {
            if (
                $_REQUEST["execute_mode"] == "MAINTAIN" && $this->allow_maintain != "SAFE"
    ...
    public function setRequestColumns()
    {
        ...
        */
        ...
        if (
            $sessionClass::getReporticoSessionParam("firstTimeIn")
            ...
            if ($this->execute_mode == "EXECUTE") {
                ...
                if (
                    !$this->target_show_detail && !$this->target_show_graph && !$this->target_show_group_headers
    ...
    // -----------------------------------------------------------------------------
    public function &buildWhereExtraList($criteria_name, $in_is_expanding = false)
    {
        $str = "";
        $expval = false;
        if ($in_is_expanding) {
    ...
    // -----------------------------------------------------------------------------
    public function setGroupTrailerByNumber($query_name, $trailer_number, $trailer_column, $value_column, $show_in_html, $show_in_pdf, $trailer_custom = false)
    {
        $tn = (int)$trailer_number;
        if (!$this->checkGroupNameR("createGroupTrailer", $query_name)) {
            ReporticoApp::handleError("Failure in Group Column Trailer: Unknown Group Name $query_name");
    ...
    public function createGroupHeader($query_name, $header_column, $header_custom = false, $show_in_html = "yes", $show_in_pdf = "yes")
    {
        ...
        $grp->addHeader($col, $show_in_html, $show_in_pdf, $header_custom);
    }
            ...
            if ($col->required == "yes") {
                ...
                if (!$this->lookup_queries[$col->query_name]->column_value) {
                    if (true || ReporticoUtility::getRequestItem("new_reportico_window", false)) {
                        ...
                        ReporticoApp::handleError(
                            ReporticoLang::templateXlate("REQUIRED_CRITERIA") . " - " . ReporticoLang::translate($this->lookup_queries[$col->query_name]->deriveAttribute("column_title", "")),
                            E_USER_ERROR
                        );
                    }
                }
    ...
    public function handleXmlQueryInput($mode = false)
    {
        ...
        // definitions
        if (
            array_key_exists("submit_maintain_NEW", $_REQUEST) ||
        ...
        if (
            $this->xmlinput == "deleteproject.xml" ||
        ...
        */
        ...
        switch ($mode) {
            ...
            case "EXECUTE":
                ...
                if (
                    count(ReporticoApp::getSystemErrors()) > 0 ||
...
// print_r($query_name);exit;
        ...
        // print_r($col->column_value);exit;
        $result = str_replace(",", "", $col->column_value);
        // print_r($col->old_column_value);exit;
...
// print_r($result);exit;
    ...
    // Function : COuntnum
    // -----------------------------------------------------------------------------
    public function countnum($query_name, $group_name = false)
    {
        $i = 0;
        $i++;
        return 1;
    ...
    public function setProjectEnvironment($initial_project = false, $project_folder = "projects", $admin_project_folder = "projects")
    {
        ...
        if ($configfile) {
            ...
            if (ReporticoApp::get("included_config") && ReporticoApp::get("included_config") != $configfile) {
                ...
                // echo $configfile;
                // include_once $_SERVER['DOCUMENT_ROOT'].$configfile;
                include_once $configfile;
                ReporticoApp::set("included_config", $configfile);
            }
