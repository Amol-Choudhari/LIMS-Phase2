		...
		<ul class="nav nav-pills nav-sidebar flex-column p-0" data-widget="treeview" role="menu" data-accordion="false">
			...
			<?php if (!empty($current_user_roles)) { ?>
				...
				<?php } if($current_user_roles['administration'] == 'yes') { ?>
					...
					<li class="nav-item">
						<li class="nav-item has-treeview">
							<a href="<?php echo $this->request->getAttribute('webroot'); ?>users/user_pending_work_transfer" class="nav-link"><i class="nav-icon fas fa-poll-h"></i><p>User Work Transfer<i class="nav-icon fas fa-angle-right right"></i></p></a>
						</li>
					</li>
