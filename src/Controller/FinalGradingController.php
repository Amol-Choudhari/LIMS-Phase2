...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>--------<Grading By Inward>-------->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	public function gradingByInward(){
		...
		if (!empty($verify_sample_code)) {
			...
			//																							//	
			////::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::////
			//	/*if($this->request->getData('button')=='delete'){										//
			//																							//	
			//		$sample_code=$this->request->getData('sample_code');								//
			//																							//
			//		$tests=$this->request->getData('test_code');										//
			//																							//
			//			if($this->Final_Test_Result->updateAll(											//
			//			array('display' => "'N'"),														//
			//			array('sample_code' => $sample_code,'test_code'=>$tests)						//
			//			))																				//
			//			{																				//
			//				$this->actual_test_data->updateAll(											//
			//				array('status_flag' => "'C'"),												//	
			//					array('sample_code'=>$sample_code,'test_code'=>$tests,'display'=>'Y')	//	
			//				);																			//
			//				echo 1;																		//
			//																							//
			//			}																				//
			//			else{																			//	
			//				$errors =$this->Final_Test_Result->validationErrors;						//	
...
/***************************************************************************************************************************************************************************************/
	...
	public function getSampleCommodityGrads(){
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		///*$inward_grade = $this->Workflow1->find('first',array('joins'=>array(array('table' => 'sample_inward','alias' => 'si','type' => 'INNER','conditions' => array( 'si.org_sample_code = Workflow1.org_sample_code'))), /////
		//									 'fields'=>array('si.grade','si.sub_grad_check_iwo'),        																													   ////
		//									 'conditions'=>array('Workflow1.stage_smpl_cd'=>$sample_code)));*/																												   ///
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		///*$commodity_code_grades = $this->Comm_Grade->find('all',array('joins'=>array(array('table' => 'm_grade_desc','alias' => 'cg','type' => 'INNER','conditions' => array( 'cg.grade_code = Comm_Grade.grade_code'))),	////
		//															'fields'=>array('cg.grade_desc','cg.grade_code'),																										 	///
		//															'conditions'=>array('Comm_Grade.commodity_code'=>$commodity_code,'Comm_Grade.display'=>'Y')));*/															//
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
/******************************************************************************************************************************************************************************************************/
	...
	//added for non grade zscore view on 29-07-2022 by shreeya
	public function redirectToNonGradeZscore($sample_code){

		$this->Session->write('sample_code',$sample_code);
		$this->redirect(array('controller'=>'FinalGrading','action'=>'ilc_non_grading_by_oic'));
	}
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>??>>>>>>>--------<Save Final Grading>-------->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	public function saveFinalGrading(){
		...
		elseif ($_SESSION['user_flag']=='CAL' && $_SESSION['role']=='DOL' || $_SESSION['role']=='RAL/CAL OIC') {
			...
			// Add two new fileds to add subgrading value and oic grading date ,
			$conn->execute("UPDATE sample_inward SET status_flag='G',
													  remark ='$remark',
													  grade='$grade_code_vs',
													  remark_officeincharg ='$remark_new',
													  remark_officeincharg_dt='$tran_date',
...
/******************************************************************************************************************************************************************************************************/
	...
	// public function IlcfrdToOicZscore($sample_code){

	// 	$list = $this->ilcFrdToOicSampleZscore($sample_code);
	// 	$this->set('final_score',$list);
	// }
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>---------------<Final Sample Test Reports>--------------->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	public function finalSampleTestReports(){
		...
		if (!empty($result)) {
			...
			foreach ($result as $sample_code) {
				...
				//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				//																																																						//
				//	/*$final_grading = $this->Workflow->find('all',array('joins'=>array(array('table' =>'sample_inward','alias' => 'si','type' => 'INNER','conditions' => array('si.org_sample_code = Workflow1.org_sample_code')),		//
				...
				//														'fields'=>array('Workflow1.stage_smpl_cd','Workflow1.tran_date','mcc.category_name','mc.commodity_name','mst.sample_type_desc','mc.commodity_code'),			//
				//														'conditions'=>array('Workflow1.stage_smpl_flag'=>'FG','Workflow1.org_sample_code'=>$sample_code['Workflow1']['org_sample_code']),								//
				//														))->first();*/																																					//
				//																																																						//
				//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	
	{
		...
		$this->loadmodel('IlcCalculateZscore');
		...
		$query1 = $conn->execute("SELECT sm.ilc_org_sample_cd,w.stage_smpl_cd,w.tran_date,ro.ro_office,si.report_pdf,sm.final_zscore,sm.calculate_zscore
								FROM ilc_org_smplcd_maps AS sm
		...
		//Save the calculate Zscore on 04-08-2022 by Shreeya 
		// $this->Ilc->SaveCalculateZscore($sample_code);

			$postData = $this->request->getData();

				$getSavedList = $this->IlcOrgSmplcdMaps->find('all')->select()->where(['status IS' => '1','org_sample_code IS'=>$sample_code])->toArray();
			...
			$ZscorEntity = $this->IlcCalculateZscore->newEntity($zscore);
						
			$this->IlcCalculateZscore->save($ZscorEntity);
			


		//added for text code on 02-08-2022 by shreeya 
		$test = $this->ActualTestData->find('all', array('fields' => array('test_code'=>'distinct(test_code)'),'conditions' =>array('sample_code IS' => '5147737', 'display' => 'Y')))->toArray();
		...
		$i=0;
			...
			$this->set('teststring',$test_string);
			
			
		//added for fetch test name & method on 02-08-2022 by shreeya 
		$result3 = array();
		$i=0;
		foreach($test_string as $row1) {

			$query3 = $conn->execute("SELECT t.test_name
					FROM actual_test_data AS td
					INNER JOIN m_test AS t ON t.test_code = td.test_code
					WHERE td.sample_code= '5147737' AND td.test_code ='$row1' AND td.display = 'Y'
					ORDER BY t.test_name ASC ");
				
			$result3[$i] = $query3->fetchAll('assoc');	
		$i++;
		}
		...
		//added for final zscore forwarded to oic on 27-07-2022 by shreeya
		if ($this->request->is('post')) {
			
			if (null !==($this->request->getData('frd_to_oic'))) {

					$samplelist= $this->SampleInward->find('all', array('conditions'=> array('stage_sample_code IS' => $sample_code)))->first();
					$category_code=$samplelist['category_code'];
					$commodity_code=$samplelist['commodity_code'];
					...
					$src_usr_cd = $conn->execute("SELECT src_usr_cd FROM workflow 
										WHERE org_sample_code='$ogrsample' AND stage_smpl_flag='TA' ");
																// $ogrsample
	...
	//29-07-2022
	public function ilcNonGradingByOic()
    {
